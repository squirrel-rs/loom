...
macro_rules! assert_tokens {
    ($text:expr) => {
        let result = match std::panic::catch_unwind(|| {
            ...
            let lexer = loom_lex::lexer::Lexer::new(src.clone(), $text);
            format!("{:#?}", lexer.collect::<Vec<loom_lex::token::Token>>())
        }) {
            Ok(result) => result,
            Err(err) => {
...
macro_rules! assert_ast {
    ($text:expr) => {{
        let result = match std::panic::catch_unwind(|| {
            ...
            let src = std::sync::Arc::new(miette::NamedSource::new("test.ql", $text.to_string()));
            let lexer = loom_lex::lexer::Lexer::new(src.clone(), $text);
            let mut parser = loom_parse::parser::Parser::new(src, lexer);
...
macro_rules! assert_eval {
    ($text:expr) => {{
        let result = match std::panic::catch_unwind(|| {
            ...
            let mut interpreter = loom_rt::interpreter::Interpreter::new(&mut io);
...
macro_rules! assert_sema {
    ($text:expr) => {{
        let result = match std::panic::catch_unwind(|| {
            $crate::assertion::prepare_miette();
            let src =
                std::sync::Arc::new(miette::NamedSource::new("test.loom", $text.to_string()));
            let lexer = loom_lex::lexer::Lexer::new(src.clone(), $text);
            let mut parser = loom_parse::parser::Parser::new(src, lexer);
            let ast = parser.parse();
            let mut analyzer = loom_sema::Analyzer::default();
